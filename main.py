...
BASE=os.path.dirname(__file__); DATA_DIR=os.environ.get('DATA_DIR',BASE); os.makedirs(DATA_DIR,exist_ok=True); DB=os.path.join(DATA_DIR,'lovebirdies.db'); UPLOAD_DIR=os.path.join(DATA_DIR,'uploads')
...
def public_user(r,distance=None):
 ...
 return out
def self_user(r): out=public_user(r); out['has_location_coordinates']=r['latitude'] is not None and r['longitude'] is not None; return out
...
def blocked(c,a,b): return c.execute('SELECT 1 FROM blocks WHERE (blocker_id=? AND blocked_id=?) OR (blocker_id=? AND blocked_id=?)',(a,b,b,a)).fetchone() is not None
@app.get('/health')
...
def like(other_id:int,authorization:Optional[str]=Header(None)):
 ...
 c.execute('INSERT OR IGNORE INTO likes VALUES(?,?,?)',(uid,other_id,datetime.utcnow().isoformat())); matched=c.execute('SELECT 1 FROM likes WHERE liker_id=? AND liked_id=?',(other_id,uid)).fetchone() is not None; c.commit(); c.close(); return {'matched':matched}
